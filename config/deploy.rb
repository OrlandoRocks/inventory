...
# Aditional tasks
require_relative 'deploy/tasks/database'
require_relative 'deploy/tasks/utils'
...
set :shared_paths, ['config/database.yml', 'config/secrets.yml', 'log', 'tmp/pids', 'tmp/sockets', 'config/secrets.yml', 'config/application.yml', 'public/uploads']
...
task :setup => :environment do
  ...
  # Devise needs to read the generated secrets. Use environment variables.
  queue! %[touch "#{deploy_to}/#{shared_path}/config/secrets.yml"]
...
task :environment do
  ...
  # For those using RVM, use this to load an RVM version@gemset.
  invoke :'rvm:use[ruby-2.3.1@inventory-gemset]'
end
