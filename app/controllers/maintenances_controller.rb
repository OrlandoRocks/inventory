class MaintenancesController < ApplicationController
  ...
  def index
    @search_maintenances = Maintenance.ransack(params[:q])
    @maintenances = @search_maintenances.result.paginate(page: params[:page], per_page: 20)
  end
  ...
  def update
    respond_to do |format|
      if @maintenance.update(maintenance_params)
        format.html { redirect_to maintenances_path, notice: 'Maintenance was successfully updated.' }
  ...
  private
    ...
    def maintenance_params
      params.require(:maintenance).permit(:name, :description, :status)
    end
end
