class Api::V1::InventoryManagerController < ActionController::Base
  ...
  def get_items_not_sell
    ...
    if @user.god? or @user.admin?
      @items = Item.where(status_item_id: StatusItem.find_by_key('no_vendido')).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    else
      @items = Item.where(status_item_id: StatusItem.find_by_key('no_vendido'), department_id: @user.department_id).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    end
  ...
  def get_items_sell
    ...
    if @user.god? or @user.admin?
      ...
      @items = Item.where(status_item_id: StatusItem.where(key:['vendido', 'pendiente_factura', 'facturado']).pluck(:id)).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    else
      @items = Item.where(status_item_id: StatusItem.where(key:['vendido', 'pendiente_factura', 'facturado']).pluck(:id), department_id: @user.department_id).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    end
  ...
  def get_items_order
    @user = User.find(params[:user_id])
    if @user.god? or @user.admin?
      @items = Item.where(status_item_id: StatusItem.find_by_key('pendiente')).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    else
      @items = Item.where(status_item_id: StatusItem.find_by_key('pendiente'), department_id: @user.department_id).as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})
    end
  ...
  def update_item
    begin
      ...
      if item.update(item_params)
        ...
        p 'se actualizo!'
        render json: {status:200, success: true, message:'se actualizo el Articulo correctamente!', item: item.as_json(except: :image, include: {user: {except: [:avatar, :received_file], include: :department}})}
      else
        p 'algo valio shit'
