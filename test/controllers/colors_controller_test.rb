...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { model_part: @color.model_part, name: @color.name, status: @color.status } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { model_part: @color.model_part, name: @color.name, status: @color.status } }
